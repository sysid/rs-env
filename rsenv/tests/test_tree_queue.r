...
#[rstest]
fn test_build_trees3() -> Result<()> {
    let trees = build_trees(Utf8Path::new("./tests/resources/data"))?;
    println!("trees: {:#?}", trees);
    for tree in &trees {
        println!("Depth of tree rooted at {}: {}", tree.file_path, tree.depth3());
        assert_eq!(tree.depth3(), 5);
    }
    for tree in &trees {
        let leaf_nodes = tree.leaf_nodes3();
