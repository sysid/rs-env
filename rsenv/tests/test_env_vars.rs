...
fn temp_dir_with_env_vars() -> (PathBuf, String) {
    ...
    fs::write(&child_path, "# rsenv: $RSENV_TEST_DIR/parent.env\nexport CHILD_VAR=child_value\n").unwrap();
    
    // Create child file with ${VAR} syntax
    let child_path2 = temp_path.join("child2.env");
    fs::write(&child_path2, "# rsenv: ${RSENV_TEST_DIR}/parent.env\nexport CHILD2_VAR=child2_value\n").unwrap();
    
    (temp_dir.into_path(), env_var_name.to_string())
}

#[rstest]
fn given_env_var_in_path_when_extracting_env_then_expands_variables(temp_dir_with_env_vars: (PathBuf, String)) -> TreeResult<()> {
    ...
    assert_eq!(all_vars.get("PARENT_VAR"), Some(&"parent_value".to_string()));
...
fn given_nonexistent_env_var_when_extracting_env_then_handles_gracefully() -> TreeResult<()> {
    ...
    let child_path = temp_path.join("child.env");
    fs::write(&child_path, "# rsenv: ${RSENV_NONEXISTENT_VAR}/parent.env\nexport CHILD_VAR=child_value\n").unwrap();
