//! Tree visualization traits for converting arena trees to displayable format.
//!
...
/// Trait for converting tree structures to displayable `termtree::Tree`.
pub trait TreeNodeConvert {
    /// Convert the tree to a `termtree::Tree<String>` for pretty-printing.
                ...
                if let Some(node) = arena.get_node(node_idx) {
                    for &child_idx in &node.children {
                        if let Some(child) = arena.get_node(child_idx) {
                            let mut child_tree =
                                Tree::new(child.data.file_path.display().to_string());
    ...
    if let Some(node) = tree.get_node(node_idx) {
        ...
        for &child_idx in &children {
            if let Some(child_node) = tree.get_node(child_idx) {
                let mut child_tree =
                    Tree::new(child_node.data.file_path.to_string_lossy().to_string());
