//! Arena-based tree structures for environment hierarchies.
//!
...
impl TreeArena {
    ...
    /// Collects all branches (leaf-to-root paths) in the tree.
    ///
    /// Each branch is a Vec<PathBuf> starting from a leaf and ending at the root.
    /// This is useful for displaying the hierarchy from leaf's perspective.
    #[instrument(level = "debug", skip(self))]
    pub fn branches(&self) -> Vec<Vec<PathBuf>> {
        let mut result = Vec::new();
        if let Some(root) = self.root {
            self.collect_branches(root, &mut Vec::new(), &mut result);
        }
        result
    }
    ...
    fn collect_branches(
        ...
        if let Some(node) = self.get_node(node_idx) {
            ...
            current_path.push(node.data.file_path.clone());

            if node.children.is_empty() {
                // This is a leaf - we have a complete branch
                ...
                for &child in &node.children {
                    self.collect_branches(child, current_path, result);
                }
            }
...
/// Create branches from multiple trees.
///
/// Collects all leaf-to-root paths from all trees.
/// Each branch is a Vec<PathBuf> starting from a leaf and ending at the root.
pub fn create_branches(trees: &[TreeArena]) -> Vec<Vec<PathBuf>> {
    let mut result = Vec::new();
    for tree in trees {
        result.extend(tree.branches());
    }
    result
}
