...
impl SopsService {
    ...
    /// Update .gitignore with managed section for plaintext patterns.
    ///
    /// Adds patterns between markers:
    /// ```text
    /// # rsenv-managed start
    /// *.env
    /// *.envrc
    ...
    ///
    /// # Arguments
    /// * `base_dir` - Directory containing .gitignore
    pub fn update_gitignore(&self, base_dir: &Path) -> ApplicationResult<()> {
        let gitignore_path = base_dir.join(".gitignore");
        let start_marker = "# rsenv-managed start";
        let end_marker = "# rsenv-managed end";
        ...
        let cleaned = Self::remove_managed_section(&existing, start_marker, end_marker);

        // Build new managed section
        let mut patterns = Vec::new();
        for ext in &self.settings.sops.file_extensions_enc {
            patterns.push(format!("*.{}", ext));
        }
        for name in &self.settings.sops.file_names_enc {
        ...
        self.fs.write(&gitignore_path, &new_content).map_err(|e| {
            ApplicationError::OperationFailed {
    ...
    pub fn clean_gitignore(&self, base_dir: &Path) -> ApplicationResult<()> {
        let gitignore_path = base_dir.join(".gitignore");
        let start_marker = "# rsenv-managed start";
        let end_marker = "# rsenv-managed end";
        ...
        let cleaned = Self::remove_managed_section(&content, start_marker, end_marker);
