...
impl SopsService {
    ...
    /// Migrate old format .enc files to new hash-based format.
    ///
    /// For each old format file ({name}.enc):
    /// 1. Decrypt to get plaintext
    /// 2. Compute hash
    /// 3. Rename to {name}.{hash}.enc
    ///
    /// # Arguments
    /// * `base_dir` - Directory to process
    ///
    /// # Returns
    /// Vec of (old_path, new_path) for migrated files
    pub fn migrate(&self, base_dir: Option<&Path>) -> ApplicationResult<Vec<(PathBuf, PathBuf)>> {
        let dir = base_dir
            .map(PathBuf::from)
            .unwrap_or_else(|| self.settings.vault_base_dir.clone());
        ...
        // Find all .enc files
        let enc_files = self.collect_files(&dir, &["enc".into()], &[])?;
        ...
        for enc_path in enc_files {
            ...
            // Old format: {name}.enc
            if !is_old_enc_format(&enc_path) {
                continue;
            }

            debug!("migrate: processing old format: {}", enc_path.display());

            // Decrypt to temp location to get content
            let plaintext_path = self.decrypt_file(&enc_path)?;

            // Compute hash of decrypted content
            let hash = file_hash(&plaintext_path)?;

            // New encrypted filename
            let new_enc_path = encrypted_filename(&plaintext_path, &hash);

            // Rename old .enc to new format
            self.fs.rename(&enc_path, &new_enc_path).map_err(|e| {
                ApplicationError::OperationFailed {
                    context: format!(
                        "rename {} to {}",
                        enc_path.display(),
                        new_enc_path.display()
                    ),
                    source: Box::new(e),
                }
            })?;

            migrated.push((enc_path, new_enc_path));
        }

        debug!("migrate: migrated {} files", migrated.len());
        Ok(migrated)
    }
}
